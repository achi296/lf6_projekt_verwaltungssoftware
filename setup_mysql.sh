...
mysql -h localhost --username=root --password=$1 < database/create_database.sql
# create a new user "pyuser" and set the permissions
echo -e "Creating a new user pyuser\n"
mysql -h localhost --username=root --password=$1 < database/setup_permissions.sql
# create the tables
declare -i I=1
...
do
  ...
  mysql -h localhost --username=root --password=$1 < $SCRIPT
  I=$I+1
done